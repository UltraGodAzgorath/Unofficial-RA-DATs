...
cls
echo ============================================================
echo CHD Auto Converter - Running
echo ============================================================
echo.
...
if /I "%MODE%"=="CD_DEFAULT" (
    "%CHDMAN%" createcd -i "%INPUT%" -o "%OUTPUT%" >>"%LOG%" 2>&1
    goto CheckResult
)

if /I "%MODE%"=="CD_ZSTD" (
    "%CHDMAN%" createcd -i "%INPUT%" -o "%OUTPUT%" -c zstd,flac >>"%LOG%" 2>&1
    goto CheckResult
)

if /I "%MODE%"=="DVD_ZSTD" (
    "%CHDMAN%" createdvd -i "%INPUT%" -o "%OUTPUT%" -c zstd >>"%LOG%" 2>&1
    goto CheckResult
)
